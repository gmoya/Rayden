...
abstract class BaseProvinciaPeer {
	 ...
	 */
	public static function addInstanceToPool(Provincia $obj, $key = null)
	{
	...
	{
		return $withPrefix ? ProvinciaPeer::CLASS_DEFAULT : ProvinciaPeer::OM_CLASS;
	}
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseProvinciaPeer', $values, $con))
      {
		...
		if ($criteria->containsKey(ProvinciaPeer::ID) && $criteria->keyContainsValue(ProvinciaPeer::ID) ) {
			throw new PropelException('Cannot insert a value for auto-increment primary key ('.ProvinciaPeer::ID.')');
		}
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseProvinciaPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseProvinciaPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseProvinciaPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseProvinciaPeer', $values, $con, $ret);
    }
