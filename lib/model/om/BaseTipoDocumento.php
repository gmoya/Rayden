...
/**
 * Base class that represents a row from the 'tipo_documento' table.
 *
 * 
 *
...
abstract class BaseTipoDocumento extends BaseObject  implements Persistent {
	 ...
	 * @return     TipoDocumento The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = TipoDocumentoPeer::NUM_COLUMNS - TipoDocumentoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTipoDocumento:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTipoDocumento:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTipoDocumento:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTipoDocumento:save:post') as $callable)
				{
	 ...
	 * Otherwise if this TipoDocumento has previously been saved, it will retrieve
	 * related Personas from storage. If this TipoDocumento is new, it will return
		...
		if ($this->collPersonas === null) {
			if ($this->isNew()) {
			   $this->collPersonas = array();
				...
				$criteria->add(PersonaPeer::TIPO_DOCUMENTO_ID, $this->id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseTipoDocumento:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseTipoDocumento::%s', $method));
	  }
