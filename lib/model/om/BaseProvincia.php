...
abstract class BaseProvincia extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Localidad[] Collection to store aggregation of Localidad objects.
	 */
	protected $collLocalidads;

	/**
	 * @var        Criteria The criteria used to select the current contents of collLocalidads.
	 */
	private $lastLocalidadCriteria = null;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = ProvinciaPeer::NUM_COLUMNS - ProvinciaPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastLocalidadCriteria = null;
	 ...
	 * related Domicilios from storage. If this Provincia is new, it will return
		...
		if ($this->collDomicilios === null) {
			if ($this->isNew()) {
			   $this->collDomicilios = array();
				...
				$criteria->add(DomicilioPeer::PROVINCIA_ID, $this->id);
	...
	public function getDomiciliosJoinPersona($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collDomicilios === null) {
			if ($this->isNew()) {
				...
				$this->collDomicilios = DomicilioPeer::doSelectJoinPersona($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collLocalidads collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	/**
	 * Initializes the collLocalidads collection (array).
	 *
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Provincia has previously been saved, it will retrieve
	 * related Localidads from storage. If this Provincia is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array Localidad[]
	 * @throws     PropelException
	 */
		...
		if ($this->collLocalidads === null) {
			if ($this->isNew()) {
			   $this->collLocalidads = array();
				...
				$criteria->add(LocalidadPeer::PROVINCIA_ID, $this->id);
	...
	public function countLocalidads(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collLocalidads === null) {
			if ($this->isNew()) {
				...
				$count = LocalidadPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastLocalidadCriteria) || !$this->lastLocalidadCriteria->equals($criteria)) {
					...
					$count = count($this->collLocalidads);
				}
			} else {
				$count = count($this->collLocalidads);
			}
	...
	public function addLocalidad(Localidad $l)
	{
		...
		if (!in_array($l, $this->collLocalidads, true)) { // only add it if the **same** object is not already associated
			array_push($this->collLocalidads, $l);
			$l->setProvincia($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collLocalidads) {
				foreach ((array) $this->collLocalidads as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
