...
abstract class BaseDocente extends BaseObject  implements Persistent {
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->created_by_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->updated_at = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->updated_by_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->deleted_at = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->deleted_by_id = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			$this->resetModified();
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getCreatedById(),
			$keys[4] => $this->getUpdatedAt(),
			$keys[5] => $this->getUpdatedById(),
			$keys[6] => $this->getDeletedAt(),
			$keys[7] => $this->getDeletedById(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setCreatedById($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setUpdatedAt($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setUpdatedById($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setDeletedAt($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setDeletedById($arr[$keys[7]]);
	}
